...
$(function () {
	...
	$('.slimScrollDiv').height($(window).height() - 51);
	...
	$(document).ready(function () {
		...
		if($('#change_password_btn').length > 0) {
			$('#change_password_btn').on('click', function(e) {
				...
				var input = $('#change_password_form .validate-input');
		...
		if($('#add_wallet_btn').length > 0) {
			$('#add_wallet_btn').on('click', function () {
				...
				$.ajax({
					...
					dataType: 'json',
					data: {
						'currency_id': $('#new_wallet_currency_id').val().trim(),
						'wallet_name': $('#new_wallet_wallet_name').val().trim()
					},
					success: function (data, textStatus, xhr) {
						...
						if (data.data.success && data['errors'].length == 0) {
							...
							$(this).html('Add wallet <i class="fa fa-arrow-right"></i>');
							NOTIFY.show('Something is wrong, please try again later.', 'error');
						}
					}
				});
...
$('.form-control.validate').each(function(){
    ...
    function showValidate(input) {
        var thisAlert = $(input);
        $(thisAlert).parent().find('label span').html('<i class="fa fa-info red-text"></i>').addClass('error');
    ...
    function hideValidate(input) {
        var thisAlert = $(input);
        $(thisAlert).parent().find('label span').html('').removeClass('error');
