...
class Xannia_auth
{
	...
	public function __construct()
	{
		...
		$email_config = $this->config->item('email_config', 'xannia');

		if ($this->config->item('use_ci_email', 'xannia') && isset($email_config) && is_array($email_config))
		{
	...
	public function __call($method, $arguments)
	{
		...
		{
			throw new Exception('Undefined method xannia_auth::' . $method . '() called');
		}
	...
	{
		if ($this->auth_model->forgotten_password($identity))
		{
			...
			if ($user)
			{
				...
				if (!$this->config->item('use_ci_email', 'xannia_auth'))
				{
				...
				{
					$message = $this->load->view($this->config->item('email_templates', 'xannia_auth') . $this->config->item('email_forgot_password', 'xannia_auth'), $data, TRUE);
					$this->email->clear();
					$this->email->from($this->config->item('admin_email', 'xannia_auth'), $this->config->item('site_title', 'xannia_auth'));
					$this->email->to($user->email);
					$this->email->subject($this->config->item('site_title', 'xannia_auth') . ' - ' . $this->lang->line('email_forgotten_password_subject'));
	...
	public function forgotten_password_complete($code)
	{
		...
		$identity = $this->config->item('identity', 'xannia_auth');
		...
		if ($new_password)
		{
			...
			{
				$message = $this->load->view($this->config->item('email_templates', 'xannia_auth').$this->config->item('email_forgot_password_complete', 'xannia_auth'), $data, true);

				$this->email->clear();
				$this->email->from($this->config->item('admin_email', 'xannia_auth'), $this->config->item('site_title', 'xannia_auth'));
				$this->email->to($profile->email);
				$this->email->subject($this->config->item('site_title', 'xannia_auth') . ' - ' . $this->lang->line('email_new_password_subject'));
				$this->email->message($message);
	...
	public function forgotten_password_check($code)
	{
		...
		{
			if ($this->config->item('forgot_password_expiration', 'xannia_auth') > 0)
			{
				//Make sure it isn't expired
				$expiration = $this->config->item('forgot_password_expiration', 'xannia_auth');
				if (time() - $profile->forgotten_password_time > $expiration)
				{
					//it has expired
					$this->auth_model->clear_forgotten_password_code($code);
					$this->set_error('password_change_unsuccessful');
					return FALSE;
				}
			}
	...
	public function register($identity, $password, $email, $additional_data = array(), $group_ids = array())
	{
		...
		$email_activation = $this->config->item('email_activation', 'xannia_auth');
		...
		else
		{
			...
			{
				$message = $this->load->view($this->config->item('email_templates', 'xannia_auth').$this->config->item('email_activate', 'xannia_auth'), $data, true);

				$this->email->clear();
				$this->email->from($this->config->item('admin_email', 'xannia_auth'), $this->config->item('site_title', 'xannia_auth'));
				$this->email->to($email);
				$this->email->subject($this->config->item('site_title', 'xannia_auth') . ' - ' . $this->lang->line('email_activation_subject'));
				$this->email->message($message);
	...
	public function logout()
	{
		...
		if (get_cookie($this->config->item('identity_cookie_name', 'xannia_auth')))
		{
			delete_cookie($this->config->item('identity_cookie_name', 'xannia_auth'));
		}
		if (get_cookie($this->config->item('remember_cookie_name', 'xannia_auth')))
		{
			delete_cookie($this->config->item('remember_cookie_name', 'xannia_auth'));
		}
	...
	public function logged_in()
	{
		...
		$recheck = $this->auth_model->recheck_session();

		// auto-login the user if they are remembered
		if (!$recheck && get_cookie($this->config->item('identity_cookie_name', 'xannia_auth')) && get_cookie($this->config->item('remember_cookie_name', 'xannia_auth')))
		{
			$recheck = $this->auth_model->login_remembered_user();
		}
	...
	public function is_admin($id = FALSE)
	{
		...
		$admin_group = $this->config->item('admin_group', 'xannia_auth');
