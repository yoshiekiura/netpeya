<?php
    ...
    class Xcard_model extends CI_Model {
        ...
        public function deleteCard($card_id) {
            ...
            $query = $this->db->query($sql, array((int)$card_id));
        ...
        public function makeDefault($wallet_id) {
            ...
            if ($query > 0) {
                ...
                $query2 = $this->db->query($sql2, array($wallet_id, (int)$this->session->userdata('user_id')));
        ...
        public function addFunds($id, $amount) {
            ...
            $query = $this->db->query($sql, array((float)$amount, (int)$id));
        ...
        public function getWalletByID($id) {
            ...
            $query = $this->db->query($sql, array((int)$this->session->userdata('user_id'), $id));
        ...
        public function getWalletByCurrencyCode($currency_code) {
            ...
            $query = $this->db->query($sql, array((int)$this->session->userdata('user_id'), $currency_code));
            $wallet = $query->row_array();
        ...
        public function getUserCards($user_id) {
            $sql = "SELECT xc.id as card_id,
                    ...
                    c.simbol as wallet_currency_simbol,
                    c.name as wallet_currency_name
                 FROM xannia_cards xc INNER JOIN user_wallets uw on xc.wallet_id = uw.id INNER JOIN currencies c on c.id = uw.currency_id  WHERE uw.is_active = 1 AND xc.is_deleted = 0 AND xc.user_id = ?";
        ...
        private function createCard() {
            ...
            $query = $this->db->query($sql, array($card['number']));
