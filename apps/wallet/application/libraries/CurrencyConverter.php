...
class CurrencyConverter {
    ...
    private function saveIntoDatabase()
    {
        ...
        return array('amount' => (double)$value, 'rate' => $this->rate);
    }
    ...
    {
        $url = 'http://apilayer.net/api/live?access_key=' . $this->CI->config->item('currency_converter_key', 'currency_converter') . '&currences=' . $this->toCurrency . '&source=' . $this->fromCurrency . '&format=1';
        ...
        if ($output) {
            if (isset($output['quotes'][$this->fromCurrency . $this->toCurrency])) {
                return $output['quotes'][$this->fromCurrency . $this->toCurrency];
            }
        }
        return $this->rate = 0;
    }
