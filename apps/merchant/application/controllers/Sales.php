...
class Sales extends MY_Controller {
	...
	public function report()
	{
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST' || $_COOKIE["sale_search_filter"]) {
			...
			$data['sales'] = $this->transaction_model->getUserSales($filters, $pagination_config['per_page'], $start_index);
            $pagination_config['total_rows'] = $this->transaction_model->getTotalSalesRowsCount($filters);
            $this->pagination->initialize($pagination_config);
            $data['total_results'] = count($data['sales']);
