<?php
    ...
    class User_model extends MY_Model {
        ...
        public function addUser($user) {
            ...
            $query = $this->xannia_db->query($sql, array(
                $user['first_name'],
                $user['last_name'],
                $user['email_address'],
                $user['password']
            ...
            return $this->xannia_db->insert_id();
        }

        public function deductFunds($user_id, $amount) {
            $sql = "UPDATE users SET account_balance = account_balance - {$amount} WHERE id = {$user_id}";
        ...
        public function getUserInfo($key) {
            ...
            $user = $this->xannia_db->query($sql)->row_array();

            $this->load->model('account_model');

            $merchant_accounts = $this->account_model->getUsermerchant_accounts($user['id']);

            if($user && $merchant_accounts) {
                $result['user'] = $user;
                $result['merchant_accounts'] = $merchant_accounts;
            }
        ...
        public function getUserExtraData($id) {
            ...
            $query = $this->xannia_db->query($sql, array((int)$user['country_id']));
