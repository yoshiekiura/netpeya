    ...
    <div class="row">
        <div class="container">
                ...
                <?php if(count($summary_entries) > 0): ?>
                    ...
                    <table class="table table-hover">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Gross Income (<strong><?= $this->user['currency']['code'] ?></strong>)</th>
                                <th>Charges + Payouts + Refunds (<strong><?= $this->user['currency']['code'] ?></strong>)</th>
                                <th>Net Income (<strong><?= $this->user['currency']['code'] ?></strong>)</th>
                                <th>Day Balance (<strong><?= $this->user['currency']['code'] ?></strong>)</th>
                            </tr>
                        </thead>
                        <tbody>
                        <?php foreach($summary_entries as $entry): ?>
                            <tr class="sale-entry">
                                ...
                                <td><?= number_format(($entry['total_charges'] + $entry['payouts']), 2, '.', ' ') ?></td>
                                <td><?= number_format($entry['gross_income'] - ($entry['total_charges'] + $entry['payouts']), 2, '.', ' ') ?></td>
                                <td><?= number_format($entry['gross_income'] - ($entry['total_charges'] + $entry['payouts']), 2, '.', ' ') ?></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
            ...
            <div class="col-md-2 right-sidebar search">
                ...
                <form class="side-filter-form" action="/balance/summary" method="post">
