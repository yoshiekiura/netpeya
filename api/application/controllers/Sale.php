...
class Sale extends MY_Controller {
    ...
    public function init() {
       if ($this->input->server('REQUEST_METHOD') == 'POST') {
       		if(!$this->validateCompusorySaleFields()['success']) {
       			$this->response_status = $this->validateCompusorySaleFields()['response'];
    ...
    private function proccessCardPayment() {
    	if(!$this->validateCardFields()['success']) {
            ...
            $bt_request = [
                ...
                // 'descriptor' => [
                //     'name' => $this->request['descriptor']['company_name'],
                //     'url' => $this->request['descriptor']['website']
                // ]
            ];
            ...
            $bt_res = Braintree_Transaction::sale($bt_request);
            ...
            if($bt_res->success == true && strtolower($bt_res->transaction->processorResponseText) == 'approved') {
                ...
                $method_by_card_type = $this->paymentmethod_model->getMethodBySlag(strtolower($bt_log_card['cardType']));
            	$sale = array(
            		'merchant_id' => $this->merchant_info['id'],
                    'transaction_type_id' => 1,
					'payment_method_id' => $method_by_card_type['id'],
                ...
                $this->merchantsale_model->saveLog($bt_res);

            	if($sale_res) {
            		$this->merchant_model->addFunds($amount_after_charges, $this->merchant_info['id']);
    ...
    private function validateCardFields() {
    	...
    	} elseif(array_key_exists('card_number', $this->request)) {
    	...
    	if(!array_key_exists('card_expiry_month', $this->request)) {
    		...
    		if(!is_numeric($this->request['card_expiry_month'])) {
    			return array('success' => false, 'response' => Response::CARD_EXP_MONTH_FIELD_WRONG_TYPE);
    		}

            $this->request['card_expiry_month'] = (string)str_pad($this->request['card_expiry_month'], 2, '0', STR_PAD_LEFT);
    	}
    	...
    	if(!$this->validateXanniaFields()['success']) {
            ...
            if($this->customer) {
                ...
                if(!$wallet_to_use['wallet']) {
                    ...
                    $description = 'Payment to ' . $this->merchant_info['business_name'] . ' [ ' . $this->request['currency'] . number_format($this->request['amount'], 2, '.', ' ') . ']';

                    $res = $this->customertransaction_model->sale(
                        $this->customer['user']['id'],
                        Customertransaction_model::MONEY_OUT,
                    ...
                    if($res) {
                        ...
                        $this->postmark->message_html($this->load->view('templates/email/payment_success', $email_data, true));
                        $this->postmark->send();
    ...
    private function selectXanniaWallet($customer_settlement_amount, $user_wallets, $customer_settlement_currency) {
        ...
        foreach($user_wallets as $wallet) {
            if($wallet['wallet_currency_code'] == $customer_settlement_currency && $wallet['wallet_balance'] >= $customer_settlement_amount) {
        ...
        if(!$found_wallet) {
            foreach($user_wallets as $wallet) {
                if($wallet['wallet_is_default'] == 1 && $wallet['wallet_balance'] >= $customer_settlement_amount) {
    ...
    private function validateCompusorySaleFields() {
    	...
    	} elseif(array_key_exists('amount', $this->request)) {
    	...
    	} elseif(array_key_exists('currency', $this->request)) {
        ...
        // if(!array_key_exists('descriptor', $this->request)) {
        //     return array('success' => false, 'response' => Response::DESCRIPTOR_MISSING);
        // } else {
        //     if(!array_key_exists('company_name', $this->request['descriptor'])) {
        //         return array('success' => false, 'response' => Response::DESCRIPTOR_COMPANY_NAME_MISSING);
        //     }
        //     if(!array_key_exists('product_name', $this->request['descriptor'])) {
        //         return array('success' => false, 'response' => Response::DESCRIPTOR_PRODUCT_NAME_MISSING);
        //     }
        //     if(!array_key_exists('website', $this->request['descriptor'])) {
        //         return array('success' => false, 'response' => Response::DESCRIPTOR_WEBSITE_MISSING);
        //     }

        //     $this->request['descriptor']['company_name'] = substr($this->request['descriptor']['company_name'], 0, 3);
        //     $this->request['descriptor']['product_name'] = substr($this->request['descriptor']['product_name'], 0, 7);
        // }
